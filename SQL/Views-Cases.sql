...
select id_map.Model_Name, id_map.Model_ID
from id_map
inner join model_map
on  id_map.Model_Type_ID = model_map.Model_Type_ID
where model_map.Model_Type_Name ="Linear";
...
where MM.Model_Type_Name = "Logistic";
...
select LDR.Model_ID, LDR.Model_Names, LDR.RMSE, LDR.RMSE, LDR.MSE, LDR.MAE, LDR.RMSLE
from Leaderboard_metadata as LDR_MData
inner join Model_Run as MR on LDR_MData.Run_ID = MR.Run_ID
inner join Leaderboard as LDR on LDR_MData.Model_ID = LDR.Model_ID
where LDR_MData.Run_Time = 500;
...
select Model_ID, Presence_StackedEnsemble_Best_of_Family
from ID_Map where Model_Name = "GBM_1_AutoML_20190410_190849";
...
where  DM.Dataset_ID = 1 and Run_ID = "XBL7h1dtl";
