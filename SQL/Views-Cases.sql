# 1) To view the basic details about the given Dataset - dataset_basicdetails
...
# 2) List the different datasets present in given domain - dataset_domain
...
# 3) List the different runs and their details for which a dataset was tested - run_details
...
# 4) Display all the models present in the database for a particular Model-Type (GBM/GLM/Linear etc.) - models_modeltype
...
# 5) Check the default values of all hyperparameters of a particular model type - hyperparameter_defaultvalue_modules
...
# 6) Display the different models present in the BestofFamily out of all models generated in that run - models_bestoffamily
...
# 7) Display all the performance measures for all the models of a particular run - model_performancemeasures
select LDR.Model_ID, LDR.Model_Name, LDR.RMSE, LDR.MSE, LDR.MAE, LDR.RMSLE
...
# 8) Display top 10 datasets with least no. of null values in all the columns - dataset_least_null_top10
...
# 9) Display top 10 datasets with max no. of unique values in all the columns - dataset_max_unique_top10
...
# 10) List the max repeated model for a particular run - max_repeated_model
...
# 11) Display the No. of Datasets present in each domain - dataset_presence_eachdoamin
...
# 12) Check if a particular model is present in the BestofFamily for that run - presenceofmodel_bestoffamily
select Model_ID, Model_Name,Presence_StackedEnsembled_Best_of_Family
from leaderboard 
where Model_Name = "GBM_1_AutoML_20190410_190849";

# 13) Display the max. no. of models generated for a dataset in a particular run - dataset_maxnumberofmodel_run
...
# 14) List top 5 datasets with max no of observations decided based on a metric - dataset_maxobservation_top5
select dataset_ID, (Number_of_Rows* Number_of_columns) as obser 
from dataset_metadata 
order by obser desc
LIMIT 5;

# 15) List no. of variables of each data-type present in a given dataset - dataset_datatype_count
